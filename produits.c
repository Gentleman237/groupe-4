...
void verif_char(char *chaine) {
    /*
     desc: verifie la validite de la chaine de caracter, entree vide ou trop longue
     arg: char chaine, la chaine a verifier
    ...
    while (!valide) {
        ...
        if (strlen(buffer) <= 1 || empty) {
            printf("\033[31mLa chaine entree est vide ou ne contient que des espaces. Veuillez ressaisir.\033[36m\n");
        } else if (strlen(buffer) - 1 < 50 ) {
            strncpy(chaine, buffer, 50);
            chaine[strcspn(chaine, "\n")] = 0;
            majuscule(chaine);
            valide = 1;
        } else {
            printf("\033[31mLa chaîne entrée est trop longue. Veuillez ressaisir.\033[36m\n");
        }
    }
}
...
void afficher() {
    ...
    */
    ...
    if (verif_fichier(fichier)) {
        ...
        printf("                  ____________________________________VEHICULES DISPONIBLE______________________________________\n");
        while (fgets(ligne, sizeof(ligne), fichier) != NULL) {
            if (sscanf(ligne, "%d,%49[^,],%49[^,],%d,%f", &voiture.id, voiture.nom, voiture.marque, &voiture.quantite, &voiture.prix_u) == 5) {
...
void enregistrer(){
    ...
    if (temp==1){
        ...
        if(!id_existe(voiture.id)){
            ...
            verif_char(voiture.nom);
            printf("entrer la marque : ");
            verif_char(voiture.marque);
            ...
            printf("\033[31m Dsl l'ID %d existe deja\033[36m\n", voiture.id);
        }
    }
    fclose(fichier);
}
    ...
    while (fgets(ligne_produit, sizeof(ligne_produit), fichier) != NULL) {
        ...
        if (sscanf(ligne_produit, "%d,%49[^,],%49[^,],%d,%f", &voiture.id, voiture.nom, voiture.marque, &voiture.quantite, &voiture.prix_u) == 5) {
            if (voiture.id == id) {
                ...
                choix=1;
                printf("voulez vous modifier le prix: 1:non (par default), 2: oui");
                choix=verif_entier();
                if(choix==2){
...
void rechercher_voiture() {
    ...
    if (choix == 1) {
        ...
        if(prix_min>prix_max){
            printf("\033[31m Intervalle incorrect!\033[36m\n");
            return;
        }
        ...
        char ligne[1024];
        int trouve = 0;
        printf("\033[32mResultat de la recherche... voiture entre %2.f $ et %2.f $\033[36m\n",prix_min,prix_max);
        while (fgets(ligne, sizeof(ligne), fichier) != NULL) {
            if (sscanf(ligne, "%d,%49[^,],%49[^,],%d,%f", &voiture.id, voiture.nom, voiture.marque, &voiture.quantite, &voiture.prix_u) == 5) {
